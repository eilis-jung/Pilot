...
void main()
{
	...
	for(int i = 1; i < 5; ++i)
	{
        result += texture(scene_sampler, in_texcoord + vec2(tex_offset.x * float(i), 0.0)).rgb * weight[i] * intensity;
        result += texture(scene_sampler, in_texcoord - vec2(tex_offset.x * float(i), 0.0)).rgb * weight[i] * intensity;
    ...
    // if (in_texcoord.x >0.0 && in_texcoord.y >0.0) {
    //     out_color = vec4(0.7, 0.7, 0.3, 1.0);;
    // } else {
