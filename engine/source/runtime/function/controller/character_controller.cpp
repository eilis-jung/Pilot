                                 ...
                                 hits))
        {
            ...
            for (auto & hit : hits)
            {
                for (auto & go : gomap)
                {
                    ...
                    {
                        TransformComponent* tfc = go.second->tryGetComponent<TransformComponent>("TransformComponent");
                        Vector3             object_horizontal_displacement =
                            (horizontal_displacement.dotProduct(hit.hit_normal) * hit.hit_normal);
